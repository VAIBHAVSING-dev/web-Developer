...
$mobile=isset($_POST['fmobile']) ? mysqli_real_escape_string($con, $_POST['fmobile']) : '';
if($mobile == ''){
echo"0";
exit;
}
...
$row=$checkmobile ? mysqli_num_rows($checkmobile) : 0;
if($row==0){echo"0";}
else{
...
$mobile=isset($_SESSION["forgot_mobile"]) ? $_SESSION["forgot_mobile"] : '';
$sessionotp=isset($_SESSION["forgot_otp"]) ? $_SESSION["forgot_otp"] : '';
...
session_start();
@$userid=$_POST['userid'];
$fnpassword= isset($_POST["fnpassword"]) ? $_POST["fnpassword"] : '';
$fcpassword=isset($_POST["fcpassword"]) ? $_POST["fcpassword"] : '';

if($fnpassword == '' || $fnpassword !== $fcpassword)  
{
	echo"2";}else{
