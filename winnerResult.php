...
if (!isset($periodid) || $periodid == '') {
    ...
    if (!$getperiod_Query || mysqli_num_rows($getperiod_Query) == 0) {
        return;
    }
...
if (!function_exists('fallbackColorByNumber')) {
function fallbackColorByNumber($number) {
...
$switchResult=$chkmanualswitchQuery ? mysqli_fetch_array($chkmanualswitchQuery) : null;
$switchstatus=isset($switchResult['switch']) ? $switchResult['switch'] : 'no';
$switchtab=isset($switchResult['tab']) ? $switchResult['tab'] : '';

$manualresultQuery=mysqli_query($con,"select `id`,(`value`)as color,`number` from `tbl_manualresult` where `status`='1'");
$manualResult=$manualresultQuery ? mysqli_fetch_array($manualresultQuery) : null;
$manualid=isset($manualResult['id']) ? $manualResult['id'] : 0;
$manualcolor=isset($manualResult['color']) ? $manualResult['color'] : '';
$manualnumber=isset($manualResult['number']) ? $manualResult['number'] : '';
//=======================manual result end=====================================================
//----------------------------------------High Low Start----------------------------------------
$settingQuery=mysqli_query($con,"select * from `tbl_gamesettings` where `id`='1'");
$settingResult=$settingQuery ? mysqli_fetch_array($settingQuery) : null;
$settingstatus=isset($settingResult['settingtype']) ? $settingResult['settingtype'] : 'low';
$settingResult=array('settingtype'=>$settingstatus);

//------------------------------------High low End----------------------------------------------


$chkResult=mysqli_query($con,"select * from `tbl_result` where `periodid`='".$periodid."' and `tabtype` in ('parity','sapre','bcone','emerd')");
$chkResultRow=$chkResult ? mysqli_num_rows($chkResult) : 0;
if($chkResultRow==0){

$selectData=mysqli_query($con,"select * from `tbl_betting` where `periodid`='".$periodid."' and `acceptrule`='on'");
$selectdataRow=$selectData ? mysqli_num_rows($selectData) : 0;
if($selectdataRow>0)
{
if($type1=='parity')
{
$chkparityQuery=mysqli_query($con,"select * from `tbl_betting` where `periodid`='".$periodid."' and `tab`='parity'");
$chkparityRow=$chkparityQuery ? mysqli_num_rows($chkparityQuery) : 0;
if($chkparityRow>0){
...
$tempNumber=isset($tempwinResult['number']) ? $tempwinResult['number'] : rand(0,9);
$tempColor=isset($tempwinResult['color']) ? $tempwinResult['color'] : fallbackColorByNumber($tempNumber);
$tempTotal=isset($tempwinResult['total']) ? $tempwinResult['total'] : 0;
//=================random data=======================================
